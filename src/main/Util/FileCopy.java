...
public class FileCopy {
     ...
     * @param fileName 文件名
     */
    public static void copyFile(String sourceUrl, String targetUrl, String fileType, String fileName){
        File source = new File(sourceUrl);
        if(!fileType.isEmpty() && !source.getName().endsWith(fileType)) return;
        if(!fileName.isEmpty() && !source.getName().equals(fileName)) return;
        File target = new File(targetUrl);
        try(BufferedInputStream bis=new BufferedInputStream(new FileInputStream(source));
            BufferedOutputStream bos=new BufferedOutputStream(new FileOutputStream(target))) {
     ...
     * @param fileName 文件名
     */
    public static void copyDir(String sourceUrl, String targetUrl, String fileType, String fileName){
        File source = new File(sourceUrl);
        File target = new File(targetUrl);
        if(isParent(source, target)){
            System.out.println("Can't copy!");
        ...
        if(filesUrl==null){
            ...
            for(String fileUrl : filesUrl){
                ...
                String tmp2 = targetUrl + File.separator + fileUrl;
                if(new File(tmp1).isDirectory()){
                    copyDir(tmp1, tmp2, fileType, fileName);//递归调用
                }else{
                    copyFile(tmp1, tmp2, fileType, fileName); // 直接拷贝文件
                }
            }
        }

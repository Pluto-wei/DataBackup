...
public class FileController {
    ...
    public FileController(App app, FileModel fileModel, FileTreeModel treeModel) {
        ...
        fileDisplay.addUnTarListener(new UnTarListener());
        fileDisplay.addTarListener(new TarListener());
     ...
     * 打包监听类
     */
    class TarListener implements ActionListener{
        @Override
        public void actionPerformed(ActionEvent e) {
            @SuppressWarnings("unchecked")
            List<File> valuesList = fileDisplay.getSelectedValuesList();
            String name = JOptionPane.showInputDialog("input the name of tar");// 得到文件名
            fileModel.tarPack(valuesList, name);// 压缩
            fileDisplay.updateView(fileModel.getDefaultListModel()); // 更新视图
        }
    }

...
public class FileModel{
     ...
     * @param name 文件名
     */
    public void tarPack(List<File> files, String name){
        List<String> zipFiles = new ArrayList<>();
        for(File file:files ){
            zipFiles.add(file.getPath());
        }
        ...
        if(name != null){
            FilePack.pack(zipFiles, currentFile.getPath() + File.separator + name + ".tar");
            updateModels(currentFile);
        }
    }
     ...
     * @param fileList 文件列表
     */
    public void tarUnPack(List<File> fileList){
        for(File f: fileList){
            try {
                FilePack.tarUnpack(f.getPath(), currentFile.getPath());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void paste(String pos){
        for(String s : copySources){
            File tmp = new File(s);
            if(tmp.isFile()) {
                FileCopy.copyFile(s, pos + File.separator + tmp.getName());
            } else {
     ...
     * @param pos 要清空的文件列表
     */
    public void clearDir(String pos){
        File dir=new File(pos);
        defaultListModel.remove(dir);
        deleteHelp(dir);
        dir=new File(pos);
        if (!dir.exists()) {// 判断目录是否存在
            dir.mkdir();
...
//    public void addBackupSources(List<File> files, String pos) {
//
//    }
//
//    public void addRestoreSources(List<File> files, String pos) {
//
//    }
}
